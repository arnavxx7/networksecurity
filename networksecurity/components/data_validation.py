...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            logging.info(f"The dataframe has these many columns: {len(dataframe.columns)}")
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            #validate number of coulmns
            validation_status = self.validate_number_of_columns(train_df)   #checking status for train df
            if not validation_status:
                error_message = f"Train dataframe does not contain all columns. \n"
            validation_status = self.validate_number_of_columns(test_df)    #checking status for test df
            if not validation_status:
